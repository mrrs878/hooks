/*
 * @Author: your name
 * @Date: 2020-12-07 19:00:16
 * @LastEditTime: 2020-12-07 19:06:28
 * @LastEditors: Please set LastEditors
 * @Description: In User Settings Edit
 * @FilePath: \jsLibrary\react\hooks\useScreen.js
 */
...
const Screen = ({ onFinish = () => {}, proxy = "" }) => {
  const screenBorderDOM = useRef(null);
  const screenCanvasDOM = useRef(null);
    ...
    (async () => {
      if (screenState === SCREENSHOT_STATE.screened) {
        try {
          ...
          const res = await html2Canvas(document.querySelector("#container"), { proxy });
  ...
  }, [screenSize, resetEvent]);

  const onCancelScreenClick = useCallback(() => {
    reset();
  }, [reset]);
  const onFinishScreenClick = useCallback((e) => {
    reset();
  ...
  return (
    <div>
      <div id="screenBg" style={{ display: screenState === SCREENSHOT_STATE.pending? "none" : "block" }}></div>
      <div id="screenBorder" ref={screenBorderDOM} style={{
      ...
      <div draggable={true} id="screenToast" style={{
        left: screenPosition.left,
        top: screenPosition.top,
        width: screenSize.width, 
        height: screenSize.height,
        ...
        <div className="actions">
          <button onClick={onCancelScreenClick}>✖</button>
          <button onClick={onFinishScreenClick}>✔</button>
        </div>
      </div>
      <button disabled={screenState !== SCREENSHOT_STATE.pending} onClick={onScreenShotClick}>截图</button>
    </div>
  )
}
